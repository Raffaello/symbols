...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        std::make_tuple("1-1", "1 - 1"),
        std::make_tuple("1+a", "1 + a"),
        std::make_tuple("a-123", "a - 123"),
        std::make_tuple("(a-b)", "a - b"),
        std::make_tuple("(a-b + c - 1)", "((a - b) + c) - 1"),
        std::make_tuple("(a-b) + (c - 1)", "(a - b) + (c - 1)"),
        std::make_tuple("(a*b) + (c / 1)", "(a * b) + (c / 1)"),
        std::make_tuple("(a*b  +  c / 1)", "(a * b) + (c / 1)"),
        std::make_tuple("-1", "-1"),
        ...
        std::make_tuple("x=1+1", "x = 1 + 1"),
        std::make_tuple("x=1+a", "x = 1 + a"),
        std::make_tuple("1=a", "1 = a"),
        std::make_tuple("x=(1 * 10)   / 2. -+ 4.1", "x = ((1 * 10) / 2) - (+4.1)"),
        std::make_tuple("a+b = x+1", "a + b = x + 1"),    // accepting equation, the interpreter will trigger an error though
        std::make_tuple("x = -(+(-(+(-a)))) * b", "x = (-(+(-(+(-a))))) * b"),
        std::make_tuple("1^1", "1^1"),
        std::make_tuple("1^(1+1)", "1^(1 + 1)"),
        std::make_tuple("(1+1)^(1+1)", "(1 + 1)^(1 + 1)"),
        std::make_tuple("(1)^1^1^(1)", "1^(1^(1^1))"),
        std::make_tuple("x=1, x+1", "x = 1, x + 1"),
        std::make_tuple("x=1, y=2, x+y", "x = 1, y = 2, x + y")
