...
private:
    ...
    std::unordered_map<std::string, mp_t> m_table;

public:
    inline const std::unordered_map<std::string, mp_t>& table() const noexcept;

    inline void   clear() noexcept;
    inline bool   contains(const std::string& name) const noexcept;
    inline mp_t&  operator[](const std::string& name);
     ...
     */
    const char* setSymbol(const AST::INode* pNode, const mp_t& val);
    bool        getSymbol(const std::string& name, mp_t& val) const noexcept;
};
...
inline const std::unordered_map<std::string, mp_t>& SymbolTable::table() const noexcept
{
    return m_table;
}
...
inline mp_t& SymbolTable::operator[](const std::string& name)
{
    return m_table[name];
}
...
inline bool SymbolTable::getSymbol(const std::string& name, mp_t& val) const noexcept
{
