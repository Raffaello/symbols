...
private:
    ...
    std::unordered_map<std::string, ast_num_t> m_table;

public:
    inline const std::unordered_map<std::string, ast_num_t>& table() const noexcept;

    inline void       clear() noexcept;
    inline bool       contains(const std::string& name) const noexcept;
    inline ast_num_t& operator[](const std::string& name);
...
inline const std::unordered_map<std::string, ast_num_t>& SymbolTable::table() const noexcept
{
    return m_table;
}
...
inline ast_num_t& SymbolTable::operator[](const std::string& name)
{
    return m_table[name];
}
...
inline bool SymbolTable::getSymbol(const std::string& name, ast_num_t& val) const noexcept
{
